...
public:
    ...
    // 为了访问私有成员变量, 需要将BTree声明为友元类, 这样BTree就可以访问BTreeNode的私有成员变量, 从而实现插入和删除操作
...
// B树节点构造函数
template <class T>
BTreeNode<T>::BTreeNode(bool leaf) {
    isLeaf = leaf;
    n = 0;
...
void BTreeNode<T>::remove(T key) {
    ...
    if (idx < n && keys[idx] == key) {
        ...
        // 标记是否是最后一个子节点, 用于递归删除, 保证删除后的节点关键字数量不小于ORDER
        bool flag = (idx == n);
...
void BTreeNode<T>::fill(int idx) {
    ...
    if (idx != 0 && children[idx - 1]->n >= ORDER) {
        ...
        if (idx != n) {
            // 如果后一个子节点有多余的关键字, 合并子节点, 合并后的子节点关键字数量为ORDER-1，然后删除关键字
            merge(idx);
        } else {
            // 最后一个子节点合并前一个子节点
            merge(idx - 1);
...
void BTreeNode<T>::borrowFromPrev(int idx) {
    ...
    if (!child->isLeaf) {
        // 如果不是叶子节点, 将前一个子节点的最后一个子节点移到当前子节点的第一个子节点
...
// 生成DOT文件
template <class T>
void BTree<T>::visualize_tree(const std::string &filename) {
    std::ofstream file;
    file.open(filename);
    ...
    if (!node->isLeaf) {
        for (int i = 0; i <= node->n; i++) {
            if (node->children[i] != nullptr) {
                file << "node" << node << ":f" << i << " -> node" << node->children[i] << ";" << std::endl;
                generate_dot(node->children[i], file);
            } else {
                file << "null" << nullCount << " [shape=point];" << std::endl;
                file << "node" << node << ":f" << i << " -> null" << nullCount << ";" << std::endl;
                nullCount++;
            }
        }
    }
